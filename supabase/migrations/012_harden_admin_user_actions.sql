revoke insert, update, delete
  on table public.user_roles, public.account_status, public.admin_audit_log
  from service_role;
...
begin
  -- All actions share this transaction lock and revalidate after acquiring it.
  ...
  if p_action = 'set_role' then
    ...
    set status = p_status,
        ...
        suspended_until = case
          when p_status = 'suspended'::public.account_state then suspended_until
          else null
        end,
        updated_at = pg_catalog.statement_timestamp()
    where user_id = p_target_user_id;
