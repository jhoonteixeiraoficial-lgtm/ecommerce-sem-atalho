...
ALTER TABLE public.assertive_image_assets
  ...
  CHECK (origin IN ('USER_UPLOAD','ML_OWN_ITEM','ML_CATALOG','COMPETITOR','AI_GENERATED'));
...
BEGIN
  ...
  FROM pg_constraint
  WHERE conrelid = 'public.assertive_image_assets'::regclass
    AND contype = 'c'
    AND pg_get_constraintdef(oid) ILIKE '%parent_asset_id IS NOT NULL%';

  IF constraint_name IS NOT NULL THEN
    EXECUTE format('ALTER TABLE public.assertive_image_assets DROP CONSTRAINT %I', constraint_name);
  END IF;
END;
$$;

ALTER TABLE public.assertive_image_assets
  ADD CONSTRAINT assertive_image_assets_lineage_check
  CHECK (
    kind IN ('ORIGINAL_EVIDENCE','SOURCE_REFERENCE')
    OR parent_asset_id IS NOT NULL
    OR (
      kind = 'GENERATED_SCENE'
      AND origin = 'AI_GENERATED'
      AND rights_status = 'LICENSED'
      AND provider IS NOT NULL
      AND model IS NOT NULL
      AND metadata ? 'truth_brief_hash'
      AND metadata ? 'prompt_hash'
      AND metadata @> '{"review_required":true}'::jsonb
    )
  );

ALTER TABLE public.assertive_image_operations
  ALTER COLUMN input_asset_id DROP NOT NULL;
