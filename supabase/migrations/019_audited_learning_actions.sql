...
revoke insert, update, delete
  on table public.courses, public.modules, public.lessons
  from service_role;
...
begin
  ...
  if p_action = 'create' then
    if p_slug is null
      ...
      or (p_entity = 'lesson' and (p_parent_id is null or p_video_url is null or p_duration_seconds is null))
      or (p_entity = 'lesson' and (
        p_duration_seconds not between 0 and 86400
    ...
    if p_entity = 'course' then
      ...
      insert into public.lessons (
        ...
        sort_order, is_published, release_at
      ) values (
        target_id, p_parent_id, p_slug, pg_catalog.btrim(p_title), pg_catalog.btrim(p_description),
        p_video_url, p_duration_seconds, p_sort_order, p_is_published, p_release_at
      );
    end if;
